...
在 **WireDB** 中对这些 WAL 数据文件有一个统称为叫文件，这些文件有单个大小限制，当一个数据文件写满之后就会被关闭，会创建一个新的数据进行进行写入。关闭的 Region 文件会成为冷数据文件，随着数据库进行不断运行长时间运行 Region 文件会不断递增不断的占用磁盘空间，这时就需要对这些旧的 Region 文件进行压缩和定时清理。
