...
> [!WARNING]
> 上面案例最终结果是数据不一致！钱凭空消失了！按正常的业务逻辑 SQL 事务设计，事务计算是要符合 ACID 原则的，一组数据要么成功要么失败，不会出现其他状态。Redis Lua 只适合 **幂等操作** 或 **可以容忍部分失败** 的场景，不适合金融转账这种严格要求 ACID 的场景。
...
| 功能维度 | UrnaDB | Redis |
|------|--------|-------|
| ACID 支持 | ✅ 完整支持 | ❌ 不支持，Lua 只是串行执行，存在被攻击风险 |
| 原子性 | ✅ 真正的原子性 | ❌ 无回滚机制 |
