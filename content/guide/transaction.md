...
UrnaDB 的事务锁设计是基于 **Key** 级别的，对每个 Key 命名空间内的数据记录进行细粒度控制，每个 Key 对应的命名空间在初始化完成之后会自动分配一个全局锁。在默认单个原子事务请求下，UrnaDB 内部的事务处理器会使用本次事务请求中所涉及到的 Key 对应锁进行并发数据竞争的保护。使用这种方案设计的好处是多个 Key 的并发事务操作是互不影响的，能有效的提高事务处理的吞吐量，示意图如下：

<img src="/images/mutli-txns.png" alt="transaction" width="100%" />

> [!IMPORTANT]
> 在该数据结构与锁管理策略下，即使 Key 命名空间规模达到百万级，系统仍具备良好的并发扩展能力，由于事务仅对访问的 Key 加锁，而活跃事务数通常远小于 Key 空间规模，因此锁竞争概率较低，从而提升整体吞吐性能。但是在当事务涉及多个 Key 命名空间的交集运算时，基于 Key 的细粒度锁机制不再具备设计之初的并发特性和高事务吞吐量。


跨多 Key 事务在执行过程中，需要同时获取并持有多个 Key 命名空间对应的锁，从而扩大锁作用域，锁范围的扩展会提高锁冲突概率，降低并发执行效率，并最终影响系统整体吞吐性能，示意图如下：
