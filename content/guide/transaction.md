...
在高并发负载下，锁竞争会形成放大效应，使系统吞吐量下降，并成为限制事务处理能力的关键瓶颈。UrnaDB 的事务锁设计基于 **Key** 级别，对命名空间内的每条数据记录进行细粒度控制。每个 key 对应的命名空间在初始化完成之后会自动分配一个全局锁，在默认单个原子事务请求下，UrnaDB 内部的事务处理器会使用本次事务请求中所涉及到的 key 对应锁进行并发数据竞争的保护。
