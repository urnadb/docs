...
UrnaDB 的事务锁设计基于 **Key** 级别，对每个命名空间内的数据记录进行细粒度控制。每个 key 对应的命名空间在初始化完成之后会自动分配一个全局锁，在默认单个原子事务请求下，UrnaDB 内部的事务处理器会使用本次事务请求中所涉及到的 key 对应锁进行并发数据竞争的保护。使用这种方案设计的好处是多个 key 的并发事务操作是互不影响的，能有效的提高事务处理的吞吐量，示意图如下：
