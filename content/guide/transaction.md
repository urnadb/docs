...
需要注意的是在 MVCC 模式下，事务处理并不能保证每个事务都能够成功提交，当多个事务对同一数据记录产生写冲突时，系统会在提交阶段进行冲突检测并回滚冲突事务。在需要强一致性的批量事务场景下，UrnaDB 提供类似传统关系型数据库中两阶段锁 [**2PL**](https://zh.wikipedia.org/zh-cn/%E4%B8%A4%E9%98%B6%E6%AE%B5%E9%94%81) 的事务处理模式，事务串行化示意图如下：


<img src="/images/txns-2pl.png" alt="transaction" width="100%" />

在 2PL 该模式下允许事务对目标数据进行锁升级，从而获得独占访问权限，在事务执行期间，其它事务对该数据的访问请求将被阻塞，直到当前事务完成后方可继续执行。
