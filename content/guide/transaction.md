...
上述任意代码，如果被恶意用户提交到 Redis 的 Lua VM 中执行，可能会导致系统行为不可预测，从而影响整体稳定性与调试难度，例如脚本中的异常逻辑如死循环或大量内存分配可能耗尽数据库资源，使 Lua VM 长时间被占用，进而阻塞其他脚本或相关操作的正常执行。
...
> [!IMPORTANT]
> Redis 并未彻底解决 Lua 脚本带来的执行隔离问题，而是通过单线程模型、执行时间限制和受控 API，将风险从 “系统级崩溃” 降低为可观测、可干预的阻塞问题。
