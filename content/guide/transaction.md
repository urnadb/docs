...
> [!WARNING]
> 数据库的事务锁的粒度如行级、页级或表级是影响事务处理性能的关键因素，它在并发度与锁管理开销之间形成权衡：粒度越细，并发性越高但管理成本越大；粒度越粗，管理成本较低但冲突概率更高。所以影响到数据库的事务并发吞吐量关键是要素是数据库中的锁持有时间，如果同一条数据的锁持有时间越长，那么其他同样需要处理这条数据的事务就会陷入长时间阻塞状态 ⌛️ ，。
...
| 功能维度 | UrnaDB | Redis |
|------|--------|-------|
| ACID 支持 | ✅ 完整支持 | ❌ 不支持，Lua 串行，有被攻击风险 |
| 原子性 | ✅ 真正的原子性 | ❌ 无回滚机制 |
